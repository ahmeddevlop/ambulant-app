...
const EtatEntree = ({ navigation }) => {
  const dispatch = useDispatch();
  const cmdListe = useSelector((state) => state.achat);
  const { achats, loading, erreur } = cmdListe;
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      >
        ...
        <Text style={style.text}>Fournisseur</Text>
        <Text style={style.text}>Totale</Text>
      </View>
      <ScrollView>
        {achats?.length != 0 &&
          achats?.map((a, i) => (
            <Achat achat={a} index={i} navigation={navigation} />
          ))}
      </ScrollView>
    </View>
  );
